...
function compile() {
    ...
    echo -e "${CYAN}"
    make -j$(nproc) O=out ARCH=arm64 realme_3i_3_defconfig
    make -j$(nproc) O=out \
                    ARCH=arm64 \
                    CC=clang \
...
function zipping() {
    ...
    cd AnyKernel || exit 1
    zip -r9 vishalk17-rmx1821-kernel-${TANGGAL}.zip * > /dev/null 2>&1
    cd ..
    echo "Zip stored at AnyKernel/vishalk17-rmx1821-kernel-${TANGGAL}.zip"
    echo -e "${RST}"
}
compile
